...
class InverseGamma:
    ...
    def eval(self, x):
        ...
        energy = tf.reduce_sum(- tf.reduce_sum(tf.math.log(res+1e-12), 1))
        return res, energy
...
class GaussianMixtureModeling:
    ...
    def eval(self, x, p):
        ...
        energy = tf.reduce_mean(-tf.math.log(likelihood))
    ...
    def vi_learning(self, x, p):
        ...
        likelihood = tf.reduce_sum(tmp, 1)
        pen_dev = tf.reduce_sum(1.0 / mixture_cov)
        posterior_z = tf.reduce_sum(tmp, 1, keepdims=True)
        posterior = tmp / posterior_z
        kl_divergence = tf.reduce_sum(p * tf.math.log(p / (posterior + 1e-12)), 1)
        energy = tf.reduce_mean(-tf.math.log(likelihood) + kl_divergence)
